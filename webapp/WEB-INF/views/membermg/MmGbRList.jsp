        ...
        $.ajax({
          ...
          success: function(result){
            ...
            for(var i=0;i<=obj_length;i++){
              ...
              $('#MmGbRTest').append(
            		  "<tr onClick=\"location.href='../gboard/gbcontent.do?gbnum="+result[i].gbnum+"'\">"
                      + "<td class='column1'>"+result[i].gbnum+"</td>"
                      + "<td class='column1'>"+result[i].gbrseq+"</td>"
                      + "<td class='column1'>"+result[i].mname+"</td>"
                      + "<td class='column2'>"+result[i].gbrcontent+"</td>"
...
<main id="main">
  ...
  <section id="course-details" class="course-details" >
    ...
    <ul class="nav nav-tabs mb-3">
        <li class="nav-item">
          <a class="nav-link" href="MmGbListPage?mnum=${m.mnum}">모임게시판</a>
        </li>
        <li class="nav-item">
      ...
      <div class="button_group">
       ...
       <i class="bi bi-journal-text" style="margin-left:1000px; font-weight : bold; width: 1500px">    총 댓글 수:  ${pm.totalCount}개</i>
        <ul class="dropdown-menu">
          <li><a class="dropdown-item" href="MmGbRListPage?pageSize=1&mnum=${mnum}">1</a></li>
          <li><a class="dropdown-item" href="MmGbRListPage?pageSize=5&mnum=${mnum}">5</a></li>
          <li><a class="dropdown-item" href="MmGbRListPage?pageSize=10&mnum=${mnum}">10</a></li>
          <li><a class="dropdown-item" href="MmGbRListPage?pageSize=15&mnum=${mnum}">15</a></li>
        </ul>
      </div>

      <table id="MmGbRTest" class="type07" style="text-align: center; margin-left:0px">
        <thead>
        <tr>
        ...
        <c:if test="${not empty MmGbRList}">
          <c:forEach var="MmGbR" items="${MmGbRList }">
            ...
            onClick="location.href='../gboard/gbcontent.do?gbnum=${MmGbR.gbnum}&gseq=${gseq}'">  
              ...
              <td class="col-sm-1"><fmt:formatDate value="${MmGbR.rdate }" pattern="yyyy-MM-dd (E) HH:mm" /></td>
            </tr>
      ...
      <div id="firstpage">
        <nav id="nav1" aria-label="Page navigation example">
          ...
          >
            <c:if test="${pm.prev}">
              <li class="page-item">
                <a class="page-link" href="MmGbRListPage?page=${pm.startPage-1}&pageSize=${cri.pageSize}&mnum=${mnum}">처음</a>
              </li>
            </c:if>
            <c:if test="${pm.prev}">
              <li class="page-item">
                <a class="page-link" href="MmGbRListPage?page=${cri.page-1}&pageSize=${cri.pageSize}&mnum=${mnum}">이전</a>
              </li>
            </c:if>
            <c:forEach var="idx" begin="${pm.startPage }" end="${pm.endPage }">
              <li class="page-item">
                <a class="page-link" href="MmGbRListPage?page=${idx }&pageSize=${cri.pageSize}&mnum=${mnum}">${idx}</a>
              </li>
            </c:forEach>
            <c:if test="${pm.next && pm.endPage > 0}">
              <li class="page-item">
                <a class="page-link" href="MmGbRListPage?page=${cri.page+1}&pageSize=${cri.pageSize}&mnum=${mnum}">다음</a>
              </li>
            </c:if>
            <c:if test="${pm.next && pm.endPage > 0}">
              <li class="page-item">
                <a class="page-link" href="MmGbRListPage?page=${pm.endPage}&pageSize=${cri.pageSize}&mnum=${mnum}">끝</a>
              </li>
            </c:if>
          </ul>
        </nav>
