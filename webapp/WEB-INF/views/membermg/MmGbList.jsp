...
<head>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      $("#MmGbSearch").on("keyup",function(){
        var mnum = "${mnum}";
        var table = document.getElementById('MmGbTest');
        $.ajax({
          url:"MmGbListRest",
          type:"GET",
          dataType:"json",
          ...
          success: function(result){
            ...
            var trlength = $('#MmGbTest tr').length;
            for(var t=trlength-1;t>0;t--){
              table.deleteRow(t);
            }
            console.log("result: " + result[0].athur);
            for(var i=0;i<=obj_length;i++){
              $('#MmGbTest tr:even').css("background","#faf7f2");
              $('#MmGbTest').append(
            		  "<tr onClick=\"location.href='../gboard/gbcontent.do?gbnum="+result[i].gbnum+"&gseq="+result[i].gseq+"'\">"
                      + "<td class='column1'>"+result[i].gbnum+"</td>"
                      + "<td class='column1'>"+result[i].mname+"</td>"
                      + "<td class='column1'>"+result[i].gbtitle+"</td>"
                      + "<td class='column2'>"+result[i].gbcontent+"</td>"
                      + "<td class='column1'>"+result[i].gbview+"</td>"
                      + "<td class='column1'>"+result[i].rdate+"</td>"
              );
            }
          },
...
<main id="main">
  ...
  <section id="course-details" class="course-details" >
    ...
    <ul class="nav nav-tabs mb-3">
        ...
        <li class="nav-item">
          <a class="nav-link" href="MmGbRListPage?mnum=${mnum}&gseq=${gseq}">모임게시판 댓글</a>
        </li>
      </ul>
      <div class="button_group">
       ...
       <i class="bi bi-journal-text" style="margin-left:1000px; font-weight : bold; width: 1500px">    총 게시글: ${pm.totalCount}개</i>
        <ul class="dropdown-menu">
          <li><a class="dropdown-item" href="MmGbListPage?pageSize=1&mnum=${mnum}">1</a></li>
          <li><a class="dropdown-item" href="MmGbListPage?pageSize=5&mnum=${mnum}">5</a></li>
          <li><a class="dropdown-item" href="MmGbListPage?pageSize=10&mnum=${mnum}">10</a></li>
          <li><a class="dropdown-item" href="MmGbListPage?pageSize=15&mnum=${mnum}">15</a></li>
        </ul>
      </div>

      <table id="MmGbTest" class="type07" style="text-align: center; margin-left:0px">
        <thead>
        <tr>
        ...
        <tbody>
        <c:if test="${empty MmGbList}">
          <tr align="center">
            <td colspan="5">데이터가 하나도 없음</td>
          </tr>
        </c:if>
        <c:if test="${not empty MmGbList}">
          <c:forEach var="MmGb" items="${MmGbList }">
            ...
            onClick="location.href='../gboard/gbcontent.do?gbnum=${MmGb.gbnum}&gseq=${MmGb.gseq}'">  
            
              <td class="col-sm-1">${MmGb.gbnum}</td>
              <td class="col-sm-1">${MmGb.mname }</td>
              <td class="col-sm-1">${MmGb.gbtitle }</td>
              <td class="col-sm-2">${MmGb.gbcontent }</td>
              <td class="col-sm-1">${MmGb.gbview }</td>
              <td class="col-sm-1"><fmt:formatDate value="${MmGb.rdate }" pattern="yyyy-MM-dd (E) HH:mm" /></td>
            </tr>
      ...
      <div id="firstpage">
        <nav id="nav1" aria-label="Page navigation example">
          ...
          >
            <c:if test="${pm.prev}">
              <li class="page-item">
                <a class="page-link" href="MmGbListPage?page=${pm.startPage-1}&pageSize=${cri.pageSize}&mnum=${mnum}">처음</a>
              </li>
            </c:if>
            <c:if test="${pm.prev}">
              <li class="page-item">
                <a class="page-link" href="MmGbListPage?page=${cri.page-1}&pageSize=${cri.pageSize}&mnum=${mnum}">이전</a>
              </li>
            </c:if>
            <c:forEach var="idx" begin="${pm.startPage }" end="${pm.endPage }">
              <li class="page-item">
                <a class="page-link" href="MmGbListPage?page=${idx }&pageSize=${cri.pageSize}&mnum=${mnum}">${idx}</a>
              </li>
            </c:forEach>
            <c:if test="${pm.next && pm.endPage > 0}">
              <li class="page-item">
                <a class="page-link" href="MmGbListPage?page=${cri.page+1}&pageSize=${cri.pageSize}&mnum=${mnum}">다음</a>
              </li>
            </c:if>
            <c:if test="${pm.next && pm.endPage > 0}">
              <li class="page-item">
                <a class="page-link" href="MmGbListPage?page=${pm.endPage}&pageSize=${cri.pageSize}&mnum=${mnum}">끝</a>
              </li>
            </c:if>
          </ul>
        </nav>
